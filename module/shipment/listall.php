...
switch ($status)
{
	case 0 : $query_listshipment = "SELECT * FROM shipment WHERE status = 0 ORDER BY id DESC";break;
	case 1 : $query_listshipment = "SELECT * FROM shipment WHERE status = 1 ORDER BY id DESC";break;
	case 2 : $query_listshipment = "SELECT * FROM shipment WHERE status = 2 ORDER BY id DESC";break;
	case 3 : $query_listshipment = "SELECT * FROM shipment WHERE status = 3 ORDER BY id DESC";break;
	case 4 : $query_listshipment = "SELECT * FROM shipment WHERE status = 4 ORDER BY id DESC";break;
	case 9 : $query_listshipment = "SELECT * FROM shipment WHERE status = 9 ORDER BY id DESC";break;
	case 99 : $query_listshipment = "SELECT * FROM shipment WHERE car_id LIKE '-' ORDER BY id DESC";break;
	case 100 : $query_listshipment = "SELECT * FROM shipment WHERE transaction_date LIKE '".date("Y-m-d")."' ORDER BY id DESC";break;
	//case 99 : $query_listshipment = "SELECT * FROM shipment WHERE status = 99 ORDER BY id DESC";break;
	case 999 : $query_listshipment = "SELECT * FROM shipment ORDER BY id DESC";break;
	case 8888 : $query_listshipment = "SELECT * FROM shipment WHERE customer_id LIKE ".$customer_id." ORDER BY id DESC";break;
	case 9999 : $query_listshipment = "SELECT * FROM shipment WHERE product_payment_id LIKE '".$product_payment_id."'";break;
}
  ...
  <tr>
    <td align="center" bgcolor="#FFFFCC"><table width="100%" border="1" align="center" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td colspan="2" align="center" bgcolor="#FFCCFF" class="h3"><form name="form1" method="post" action="">
 ...
 <!--//สิ้นสุดเรียกข้อมูลลูกค้าเพื่อใช้ในการสร้าง Listbox ค้นหารายการสินค้า -->       
        ...
        <tr>
          ...
          <?php 
					...
					case "0" : $status_txt = "รอพิมพ์ใบส่งสินค้า"; $bg = "#FFFFFF" ; break;
					case "1" : $status_txt = "รอพิมพ์ใบตั้งเบิก"; $bg = "#FFFF00" ; break;
					case "2" : $status_txt = "รอพิมพ์ใบเสร็จรับเงิน"; $bg = "#33CCFF" ; break;
					case "3" : $status_txt = "รอพิมพ์ใบสลิปค่าจ้าง"; $bg = "#FF9900" ; break;					
					case "4" : if ($row_listshipment['rate_pricein_type2'] = 2)
								{$status_txt = "สิ้นสุดงานแล้ว"; $bg = "#00FF00" ; break;}
								else
								{$status_txt = "รอบันทึกข้อมูลการเบิกเงิน"; $bg = "#FF00FF" ; break;}
					case "9" : $status_txt = "เอกสารถูกยกเลิก"; $bg = "#FF0000" ; break;
					}
            ...
            <a href="module/shipment/detelcustomer.php?id=<?php echo $row_listshipment['id']; ?>" target="thewiondows"><ion-icon name="paper"></ion-icon></a>
            </td>
        </tr>
        
		<?php } while ($row_listshipment = mysql_fetch_assoc($listshipment)); ?>
        <tr>
          <td colspan="9" align="center" bgcolor="#FFFFFF"><table width="50%" border="0" cellspacing="0" cellpadding="0">
            <!--<tr>
            ...
            <tr>
              ...
              <?php
              	for ($dw_i=0;$dw_i<=$totalPages_listshipment;$dw_i++)
				{
					switch ($status)
					{
						case 999 : echo "<a href=?status=999&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;
						case 0 : echo "<a href=?status=0&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;
						case 1 : echo "<a href=?status=1&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;
						case 2 : echo "<a href=?status=2&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;
						case 3 : echo "<a href=?status=3&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;
						case 4 : echo "<a href=?status=4&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;
						case 9 : echo "<a href=?status=9&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;
						case 8888 : echo "<a href=?status=8888&customer_id=".$customer_id."&pageNum_listshipment=".$dw_i.">".$dw_i."</a>  ";							
//						default : echo "<a href=?pageNum_listshipment=".$dw_i.">".$dw_i."</a>  "; break;										
        ...
        <tr>
          <td colspan="9" align="center" bgcolor="#FFFFFF" class="small"><table width="80%" border="1" cellspacing="0" cellpadding="0">
            <tr>
